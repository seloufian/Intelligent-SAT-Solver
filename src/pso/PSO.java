...
public abstract class PSO {
	...
	public static Solution searchPSO(ClausesSet clset, int numParticles, int maxVelocity, int const1, int const2, int inWeight, int maxIterations, long execTimeMillis) {
		...
		for(int i=0; i<numParticles; i++) {
			particles.add(new Particle(clset.getNumberVariables(), maxVelocity));
		...
		for(int i=0; i<maxIterations; i++) {
			...
			for(Particle particle : particles) {
				particle.updateVelocity(gBest, inWeight, const1, const2, maxVelocity);
